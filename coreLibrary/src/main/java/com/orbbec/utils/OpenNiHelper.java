...
public class OpenNiHelper {
    ...
    private static final int OB6000_START_PID = 0x0600;
    //todo 摄像头id 区间设置规则 4开头 为400芯片 5开头为 color 6开头为 6000芯片
    private static final int OB6000_END_PID = 0x06FF;
    private static final int OB400_START_PID = 0x0400;
    //todo 摄像头id 区间设置规则 4开头 为400芯片 5开头为 color 6开头为 6000芯片
    private static final int OB400_END_PID = 0x04FF;
    ...
    public static boolean hasObUsbDevice(Context context) {
        ...
        while (iterator.hasNext()) {
            ...
            boolean obDevice = usbDevice.getVendorId() == OB_VID && ((usbDevice.getProductId() <= OB6000_END_PID && usbDevice.getProductId() >= OB6000_START_PID)||(usbDevice.getProductId() <= OB400_END_PID && usbDevice.getProductId() >= OB400_START_PID));
            if (obDevice) {
                return true;
            }
        }
    ...
    public UsbDevice getUsbDevice() {
        ...
        while (iterator.hasNext()) {
            UsbDevice usbDevice = iterator.next();
            Log.v(TAG, "vid: " + Integer.toHexString(usbDevice.getVendorId()) + ", pid: " + Integer.toHexString(usbDevice.getProductId()));
            boolean obDevice = (usbDevice.getVendorId() == OB_VID && (usbDevice.getProductId() <= OB6000_END_PID && usbDevice.getProductId() >= OB6000_START_PID)) ||
                    (usbDevice.getVendorId() == OB_VID && (usbDevice.getProductId() <= OB400_END_PID && usbDevice.getProductId() >OB400_START_PID));
            if (obDevice) {
                return usbDevice;
            }
        }
